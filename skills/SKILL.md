...
4. **반영** — 승인되면:
   ...
   - **COM 라이브**: 먼저 `hwp_status()`로 `available=true` 확인 → `apply_to_open_hwp(values)`로 **열린 한글 창에 원샷 반영**. Windows + 한글 필요. `needs_field_registration=true`면 문서에 누름틀(이름있는 필드)이 없다는 뜻 → 사용자에게 누름틀 등록 안내.
...
사용자 검토/승인
fill_form("강사카드.hwpx", 초안, "강사카드_채움.hwpx")   # 또는 apply_to_open_hwp(초안)
```
