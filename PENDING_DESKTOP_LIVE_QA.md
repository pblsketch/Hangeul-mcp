...
판정: 이 독립 프로브가 관측한 시점에는 실제 Explorer 더블클릭 창이 automation-visible COM 객체를 제공하지 않았다. 그러나 Claude Desktop에서의 사용자 성공 관측과 충돌하므로 current-document UX 전체를 unsupported로 판정하지 않는다. Claude Desktop의 실제 MCP 프로세스 수명 안에서 서버 시작 순서, 선행 `open_in_hwp`/COM 호출, ROT inventory, resolve/preview/apply raw JSON을 함께 재현해야 한다. 재현 전 가장 결정적인 안전 대안은 `open_in_hwp(path)`로 automation-visible 세션을 연 뒤 exact-path preview/apply를 사용하는 것이다.
