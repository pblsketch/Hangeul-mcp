...
## D20. 형성평가 출하 경계 — 실제 HWPX E2E + 품질 + artifact hygiene (G005)
- **결정**: 형성평가 workflow는 등록된 v1 profile의 실제 HWPX fixture에서 학생용·교사용·정답지 정확히 3개를 생성하는 E2E, 문서 품질 checker, 저장소 artifact hygiene checker가 모두 통과할 때만 complete로 기록한다. 생성 함수나 단위 테스트 일부의 성공만으로 출하 완료를 선언하지 않는다.
- **E2E 불변식**: 세 variant는 하나의 immutable bundle로 게시하며, source bytes/SHA와 variant 간 item evidence linkage를 보존한다. variant 하나라도 실패하면 staging/final 산출물은 0개이고, 학생용에는 teacher-only provenance edge가 없어야 한다. 등록되지 않은 template/profile inference는 fail-closed다.
- **품질·hygiene 경계**: 품질 checker는 visible placeholder, 빈 required target, marker 훼손, structural validation 실패를 구분해 거부한다. hygiene checker는 staging·journal·snapshot·생성 HWPX residue와 공개 산출물에 부적절한 로컬 provenance를 구분해 거부한다. 각 checker는 깨끗한 fixture 통과와 오염 temporary fixture 거부를 모두 증명해야 한다.
- **일반화 제한**: v1은 명시적으로 등록된 profile 하나만 지원한다. 두 번째 실제 template이 동일한 E2E·품질·publish 계약을 통과하기 전에는 filename/path pattern/fallback 기반 profile inference를 추가하지 않는다.
- **검증 추적성**: AC-E2E-01~08은 `tests/test_assessment_e2e.py`, AC-QUALITY-01은 `tests/test_assessment_quality.py`, AC-HYG-01~02는 `tests/test_assessment_hygiene.py`가 담당한다.
