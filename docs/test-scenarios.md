...
- **순서**: `open_in_hwp(사본경로)` 또는 `preview_cells_to_open_hwp(사본경로, {"필드명":"값"})`로 same-doc attach를 먼저 확인한 뒤 `apply_to_open_hwp({"필드명": "값"})`  (pathful `apply_to_open_hwp(path, values)`는 guidance/refusal-only)
- **성공**:
  - 누름틀 있음 → `applied`에 반영되고, 직전 `open_in_hwp`/preview가 가리킨 active 문서와 같은 창에 값이 들어감
