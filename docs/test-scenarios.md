...
>
> **중요**: 라이브 attach의 증거는 `connected:true`나 generic reconnect가 아니라 **정규화한 경로 exact match**입니다. 손으로 연 창도 resolver 후보일 수 있지만, 수동 검사는 `hwp_status().instances`와 `preview_cells_to_open_hwp(...)`의 attach metadata로 같은 문서임을 먼저 확인하세요. `open_in_hwp`는 가장 재현성 높은 진입점입니다.
...
- **순서**: `hwp_status()`(`instances`의 활성 문서가 사본인지 확인) → `preview_cells_to_open_hwp(...)`(attach metadata 확인) → `apply_cells_to_open_hwp(사본경로, {"성명":"홍길동","직위":"교사"})`
- **성공**:
