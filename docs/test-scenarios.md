...
자동 테스트(`pytest -q`)는 **코드가 맞는지**를 검사합니다. 현재 baseline 수치는 이 문서에 고정하지 말고 실행 결과를 함께 기록하세요.
...
2. **데스크톱 라이브 경로** — exact-path로 식별한 열린 한글 문서에 값을 넣는 기능(자동화 불가, 그룹 D)
...
>
> **중요**: 라이브 attach의 증거는 `connected:true`나 generic reconnect가 아니라 **정규화한 경로 exact match**입니다. 손으로 연 창도 resolver 후보일 수 있지만, 수동 검사는 `hwp_status().instances`와 `preview_small_live_label_cells(...)`의 attach metadata로 같은 문서임을 먼저 확인하세요. `open_in_hwp`는 가장 재현성 높은 진입점입니다.

### TS-D1. "미리보기"가 정말 아무 부작용이 없는가 (한글을 켜기 전에!)
- **왜**: preview 계열 툴은 한글 창을 띄우지도, 건드리지도 않아야 합니다.
- **순서**: 한글이 **실행되지 않은 상태**에서 `hwp_status()` → `preview_small_live_label_cells(사본, {"성명":"홍길동","직위":"교사"})`
- **성공**:
  - 두 호출 후에도 **한글 프로세스가 없음** (작업관리자로 확인)
  - status: `connected: false` + `attach_boundary`가 generic reconnect 불충분을 설명
  - preview: `count: 2`, 대상 = 성명→`t2.r2.c3`, 직위→`t2.r2.c2` + attach metadata 존재
- **증거**: 반환 JSON + 프로세스 목록

### TS-D2. 열린 한글 창의 셀이 실제로 채워지는가 (⭐ 핵심 경로)
- **준비**: `open_in_hwp(사본경로)`로 fixture **사본**을 열어 두는 것을 권장. 손으로 먼저 연 창도 가능하지만, 이후 판정은 preview의 attach metadata가 같은 정규화 경로를 가리킬 때만 진행한다.
- **순서**: `hwp_status()`(`instances`의 활성 문서가 사본인지 확인) → `preview_small_live_label_cells(...)`(attach metadata 확인) → `apply_small_live_label_cells(사본경로, {"성명":"홍길동","직위":"교사"})`
- **성공**:
  - status의 `instances` 또는 preview attach metadata가 사본과 exact-path 일치
  - apply의 `applied` 2건 == preview의 `count` (`connected:true`만으로는 합격 아님)
  - **열린 창에서 성명/직위 셀에 값이 즉시 보이고, 창이 닫히지 않음**
  - 저장 전이라면 Ctrl+Z로 되돌리기 가능
- **증거**: status/preview/apply JSON 전문 + 채워진 창 스크린샷 → **raw probe/json + literal write evidence가 함께 있을 때만** US-029 승격 + PENDING 문서 삭제
...
### TS-D4. 누름틀 문서는 exact-path로, 없으면 안내로
- **준비**: 누름틀(입력 필드)이 있는 문서를 열어 둠 ※ 기본 fixture에는 누름틀이 없어 별도 문서 필요
- **순서**: `open_in_hwp(사본경로)`로 same-doc attach를 먼저 확인한 뒤 `apply_to_open_hwp(사본경로, {"필드명": "값"})`
- **성공**:
  - 누름틀 있음 → `applied`에 반영되고, 같은 exact-path 창에 값이 들어감
  - 누름틀 없음 → `needs_field_registration: true` + 셀 방식으로 하라는 안내
- **증거**: attach 확인 JSON + apply 반환 JSON

### TS-D5. 현재 문서(pathless) UX는 saved `.hwpx`만 통과하는가
- **준비**: 저장된 `.hwpx` 현재 문서 1개 케이스와, 저장된 `.hwp` 현재 문서 1개 케이스를 각각 준비
- **순서**: `resolve_current_hwp_document()` → `preview_current_hwp_document(...)` → `apply_to_current_hwp_document(preview_token)`
- **성공**:
  - saved `.hwpx` current document → preview가 token을 발급하고 apply가 같은 문서에만 반영
  - saved `.hwp` current document → preview가 `preview_requires_hwpx`로 종료하고 token이 없음
- **증거**: resolve/preview/apply JSON + 같은 창 read-back 또는 스크린샷
---
...
| TS-E5 | `preview_small_live_label_cells(x.hwp, ...)` | `ok: false` + ".hwpx만 허용" 안내 — 부작용 없음 유지 |
...
```powershell
./.venv/Scripts/python.exe -m pytest -q             # 기대: current worktree baseline
./.venv/Scripts/python.exe -m pyflakes hangeul_core hangeul_mcp tests
./.venv/Scripts/python.exe scripts/e2e_evidence.py  # 기대: current worktree baseline
```
