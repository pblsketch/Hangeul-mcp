...
5. **남은 라이브 UX 정리** 🟡 — pathful `apply_to_open_hwp` exact-path live apply, pathless `apply_to_open_hwp(values)` legacy-only, saved `.hwpx` current-document 경로(`resolve_current_hwp_document` → `preview_current_hwp_document` → `apply_to_current_hwp_document`)의 데스크톱 QA, 그리고 `live_reload` helper 필요성/feasibility를 문서·런북·PRD에서 계속 추적.
## 이미 완료 (v0.1.0)
