...
5. **남은 라이브 UX 정리** 🟡 — pathful `apply_to_open_hwp` 우선, pathless `apply_to_open_hwp(values)` legacy-only, `open_in_hwp`의 `attached_existing` 상태와 `live_reload` helper 필요성/feasibility를 문서·런북·PRD에서 계속 추적.
## 이미 완료 (v0.1.0)
