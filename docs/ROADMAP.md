...
1. **텍스트/치환** ✅ — `search_and_replace`·`batch_replace`와 text edit-session preview/apply/restore(`preview_search_and_replace`·`preview_batch_replace`·`apply_edit_session`·`restore_edit_session`). **OWN 바이트보존**(`locate.replace_literals`, run 분할·셀 경계 가드). python-hwpx 위임 아님. (`edit.py`, `edit_session.py`, US-022)
...
2. **BYO-AI workflow 문서** ✅ — `docs/byo-ai-harness.md`에 파일 양식 채우기, 새 문서 생성, **exact-path attach-first** 열린 한글 live 흐름을 명시.
3. **Live preview gate** ✅ — `preview_small_live_label_cells`가 문서를 쓰지 않고 target table/row/col + attach metadata를 계산해 apply 전 exact-path 확인 경로 제공.
4. **No-API guard** ✅ — dev dependency와 테스트에서 서버가 LLM API SDK에 의존하지 않는다는 경계를 확인.
5. **남은 라이브 UX 정리** 🟡 — pathful `apply_to_open_hwp` exact-path live apply, pathless `apply_to_open_hwp(values)` legacy-only, saved `.hwpx` current-document 경로(`resolve_current_hwp_document` → `preview_current_hwp_document` → `apply_to_current_hwp_document`)의 데스크톱 QA, human-readable picker metadata, 그리고 `scripts/windows_live_regression_harness.py`/`docs/evidence/windows-live-regression-template.json` 기반 evidence artifact를 문서·런북·PRD에서 계속 추적.
## Deferred — known-template completion profiles

- `교수학습 지도안` 같은 알려진 양식에 대한 profile hook은 **이번 라운드에서는 보류**.
- 이유: basename/path-only 또는 SHA-only 매칭은 brittle하고, 아직 **구조 fingerprint + required-anchor set + addressed edit plan compiler**를 일반화/검증하지 못했다.
- 현재 안전 경로는 generic addressed flow 유지:
  `compact inspect once -> generate all values -> one complete_addressed_template call -> open verified output`.
- 즉, 미검증 template profile 지름길은 추가하지 않고, unknown template도 같은 addressed substrate로 fail-closed 처리한다.
## 이미 완료 (v0.1.0)

라벨:값 fill · inline-blank(마커/콜론) · 병합셀 2D occupancy · 멀티섹션 · 바이트보존 · 자간정규화 ·
MCP 서버(당시 6툴; 현재 툴 수는 README·HANDOFF 참조) · 클라이언트무관 · `.hwp` COM 변환(데스크톱) · COM 라이브 스캐폴드 · markpen 텍스트 독해 수정 · codex QA 하드닝.
