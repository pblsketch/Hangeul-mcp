...
- `apply_to_open_hwp(path, values)` / `apply_to_open_hwp(values)` — 누름틀(named field) 반영. **안전한 Batch A 흐름은** 먼저 `open_in_hwp(path)`(또는 `preview_cells_to_open_hwp(path, values)`로 attach 확인) 후, 이미 제어 중인 active 문서에 `apply_to_open_hwp(values)`를 호출하는 것이다. path 인자는 exact-path 단독 쓰기 증명이 아니라 guidance/보수적 refusal 용도이며, 누름틀 없으면 `needs_field_registration` 반환
...
- **Batch A (승인된 보수 범위, 지금 문서화해도 되는 것)**
  ...
  - `open_in_hwp(path)` / `preview_cells_to_open_hwp(path, values)`로 먼저 exact-path attach를 확인하고, named field는 legacy active-document `apply_to_open_hwp(values)`로 반영하는 **attach-first** 안내
