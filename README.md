...
MCP 클라이언트 등록은 [`docs/clients/`](docs/clients/README.md) 참고. 파이썬 코어 직접 사용:

```python
from hangeul_core.understand import understand
from hangeul_core.fill import fill

for f in understand("강사카드.hwpx").fields:
    print(f.field_id, f.label, f.kind)          # 양식 인식

fill("강사카드.hwpx", {"성명": "홍길동"}, "채움.hwpx")   # 서식 보존 채우기
```
