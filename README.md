...
**상태: v0.1.0 + Phase A~D 핵심** — v1 헤드리스(인식 · 채우기 · MCP 서버) **완성**, Phase A 양식 인식·채우기 심화 5종, Phase B 신뢰성·검증·읽기 자체코어 4종, Phase C 텍스트치환(OWN)·구조편집(python-hwpx 위임), Phase D 문서생성(위임) **완성**, v2 COM 라이브 반영은 **코드 완성 + 데스크톱 실검증 대기**. 테스트 118 passed(python-hwpx `delegate` extra 포함; 미설치 시 위임 테스트는 skip) · 독립 codex QA 통과([`docs/qa-codex-v0.1.0.md`](docs/qa-codex-v0.1.0.md)).
...
│  ├─ delegate.py           #   python-hwpx 위임 (편집·생성·리치 내보내기, optional)
...
├─ hangeul_mcp/server.py    # FastMCP stdio 서버 (19 tools: 14 OWN + 5 위임)
├─ skills/SKILL.md          # 검토→반영 Agent Skill
├─ tests/                   # 118 tests + fixtures (PII 없는 빈 양식)
