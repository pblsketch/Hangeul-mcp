...
**상태: v0.1.0 + Phase A(P0)** — v1 헤드리스(인식 · 채우기 · MCP 서버) **완성**, Phase A 양식 인식·채우기 심화 5종(형광펜·체크박스·{placeholder}·누름틀 헤드리스·form-fit) **완성**, v2 COM 라이브 반영은 **코드 완성 + 데스크톱 실검증 대기**. 테스트 81 passed · 독립 codex QA 통과([`docs/qa-codex-v0.1.0.md`](docs/qa-codex-v0.1.0.md)).
...
- `fill_form(path, values, out_path, normalize_spacing, checkbox_exclusive, auto_fit)` — 서식 보존 채우기 → 새 .hwpx. 위 모든 kind 처리. `auto_fit`은 셀 넘침 추정 시 해당 run 글꼴만 축소(하한 보장)
...
pip install -e ".[dev]"
pytest -q                     # 81 passed, 1 skipped(라이브 COM)
```
...
- 다음: Phase B(검증·읽기), Phase C/D(편집·생성, python-hwpx 위임) — [`docs/ROADMAP.md`](docs/ROADMAP.md).

마일스톤·유저 스토리(18개, 전부 pass): [`docs/prd.json`](docs/prd.json) · 설계 결정: [`docs/DECISIONS.md`](docs/DECISIONS.md) · 아키텍처: [`docs/architecture.md`](docs/architecture.md).
...
│  ├─ fill.py               #   서식 보존 채우기 (set/append/inline/placeholder/markpen/checkbox/누름틀, 멀티섹션)
│  ├─ extract.py            #   텍스트 추출
│  ├─ convert.py            #   .hwp → .hwpx (한글 COM)
│  ├─ schema.py             #   FieldSchema 데이터 모델
│  └─ hwp/com.py            #   (v2) COM 브릿지 (put_field_text)
├─ hangeul_mcp/server.py    # FastMCP stdio 서버 (7 tools)
├─ skills/SKILL.md          # 검토→반영 Agent Skill
├─ tests/                   # 81 tests + fixtures (PII 없는 빈 양식)
