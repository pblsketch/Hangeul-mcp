...
- `apply_cells_to_open_hwp(path, values, open_if_needed)` — **누름틀 없이** 셀 양식을 라이브로 채움(pyhwpx, optional `live` extra). 활성 문서==path 검증, 불일치 시 자동 open(기본) — 무관 문서 오염 방지. 창을 닫지 않음
...
pip install -e ".[dev]"
pytest -q                     # 코어만 설치 시 위임/렌더 테스트는 skip · extras 설치 로컬 기준 223 passed / 1 skipped
# 편집·생성·리치 내보내기(위임) 사용:
...
마일스톤·유저 스토리(64개 — 63 pass + 라이브/스파이크 pending, [`상태 매트릭스`](docs/prd.json)): [`docs/prd.json`](docs/prd.json) · 설계 결정: [`docs/DECISIONS.md`](docs/DECISIONS.md) · QA 리포트: [`docs/qa-codex-phaseA-D.md`](docs/qa-codex-phaseA-D.md) · 아키텍처: [`docs/architecture.md`](docs/architecture.md).
...
├─ tests/                   # 224 collected + fixtures (PII 없는 빈 양식)
