...
**상태: v0.1.0 + Phase A(P0) + Phase B(신뢰성·검증·읽기)** — v1 헤드리스(인식 · 채우기 · MCP 서버) **완성**, Phase A 양식 인식·채우기 심화 5종(형광펜·체크박스·{placeholder}·누름틀 헤드리스·form-fit) **완성**, Phase B 자체코어 4종(PII 마스킹·dry-run/백업·읽기 확장·validate) **완성**, v2 COM 라이브 반영은 **코드 완성 + 데스크톱 실검증 대기**. 테스트 99 passed · 독립 codex QA 통과([`docs/qa-codex-v0.1.0.md`](docs/qa-codex-v0.1.0.md)).
...
pip install -e ".[dev]"
pytest -q                     # 105 passed, 1 skipped(라이브 COM)
```
...
마일스톤·유저 스토리(22개, 전부 pass): [`docs/prd.json`](docs/prd.json) · 설계 결정: [`docs/DECISIONS.md`](docs/DECISIONS.md) · 아키텍처: [`docs/architecture.md`](docs/architecture.md).
...
├─ hangeul_mcp/server.py    # FastMCP stdio 서버 (14 tools)
├─ skills/SKILL.md          # 검토→반영 Agent Skill
├─ tests/                   # 105 tests + fixtures (PII 없는 빈 양식)
