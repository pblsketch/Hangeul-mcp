...
# 2) 내용을 검토
# 3) 로컬 파일로 실행
powershell -ExecutionPolicy Bypass -File .\scripts\install.ps1 -Version 0.5.2 -Client all
```
...
- `-Version 0.5.2`처럼 PyPI 버전을 지정한 관리형 설치에서 versioned update와 rollback을 사용할 수 있습니다. Git checkout이나 source bootstrap 설치는 자동으로 덮어쓰지 않으며 `unsupported_install_source`로 멈추는 것이 정상입니다.
...
- 패키지 버전: `0.5.2` (Pre-Alpha)
- 런타임 MCP 도구: **59 tools**

- 최신 로컬 검증: **559 passed, 1 skipped** (+ 로컬 프로파일 한정 사전 환경 실패 6건 — 기준선 동일, 회귀 0)
