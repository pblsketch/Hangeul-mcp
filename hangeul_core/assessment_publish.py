...
class AtomicPublishAdapter:
    ...
    __slots__ = ("_device_id", "_flags", "_move", "_published", "publish_count")
    ...
    def publish(self, staging: str | Path, final: str | Path) -> None:
        ...
        if destination in self._published:
...
def _publish_directory_no_replace(staging: str | Path, final: str | Path) -> None:
    platform_atomic_publish_adapter().publish(staging, final)
...
def run_atomic_publish_probe(
    ...
    staging = root / f".hangeul-assessment-staging-probe-{nonce}"
    final = root / f".hangeul-assessment-probe-{nonce}"
    staging.mkdir()
    marker = staging / ".hangeul-assessment-owner.json"
    ...
    try:
        ...
        marker_after = final / marker.name if final.exists() else marker
